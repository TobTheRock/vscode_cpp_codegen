...
async function getDirectoriesRecursivly(
  startDirectory: string
): Promise<string[]> {
  try {
    var dirents = await fs.promises.readdir(startDirectory, {
      withFileTypes: true,
    });
  } catch {
...
class WorkspaceDirectoryFinder {
  ...
  private workSpaceFoldersChanged(event: vscode.WorkspaceFoldersChangeEvent) {
    ...
    event.removed
      ...
      .forEach((folderPath) => {
        this._workspaceRootDirectories = this._workspaceRootDirectories.filter(
          (rootDir) => rootDir !== folderPath
        );
      });
